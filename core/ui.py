...
def _potong(teks: str, lebar: int) -> str:
    """Potong ke `lebar` karakter yang terlihat tanpa memutus kode warna."""
    hasil, terlihat, i = [], 0, 0

    while i < len(teks):
        kode = _ANSI.match(teks, i)
        if kode:
            hasil.append(kode.group())
            i = kode.end()
        elif terlihat < lebar:
            hasil.append(teks[i])
            terlihat += 1
    ...
    return "".join(hasil) + ("\033[0m" if ANSI else "")


def _tulis(teks: str) -> None:
    """
    Tulis ulang baris aktif. Harus muat satu baris layar: kalau sampai wrap,
    \r hanya kembali ke awal baris terakhir dan tiap frame animasi jadi baris baru.
    """
    try:
        # Baca langsung dari terminal; env COLUMNS bisa basi setelah layar diputar
        kolom = os.get_terminal_size(sys.stdout.fileno()).columns
    except OSError:
        kolom = shutil.get_terminal_size((80, 24)).columns

    sys.stdout.write("\r\033[2K" + _potong(teks, kolom - 1))
    sys.stdout.flush()
